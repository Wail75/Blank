...
=head1 SYNOPSIS
  ...
  my $emailer = Blank::Model::Emailer->new('https://email.provider', 'token-DKJ65FAL');
...
sub send_email {
  ...
  if ($res_json && $res_json->{status} && $res_json->{status} eq 'pending') {
    ...
    say "Blank::Model::Emailer::send_mail failed, email args: " . Dumper(\%email_args);
    return 0;
  }
}
...
sub ping {
  ...
  if ($res->code == 200) {
    ...
    say 'Blank::Model::Emailer  send_email: status is not pending';
    return 0;
  }
}
